...
export default async function StoriesPage() {
  ...
  const stories = await sanityFetch<any[]>(query);
        ...
        <section className="bg-brand-white py-16 px-6">
          <div className="max-w-7xl mx-auto">
            ...
            <Link href={`/stories/${stories[0].slug}`} className={`group flex flex-col md:flex-row rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-shadow bg-brand-offwhite`}>
              <div className={`w-full md:w-1/2 min-h-[340px] ${stories[0].color || "bg-brand-rust"} relative flex items-center justify-center`}>
                {stories[0].mainImage ? (
                  <Image src={urlFor(stories[0].mainImage).url()} alt={stories[0].title} fill className="object-cover" />
                ) : (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {stories.slice(1).map((story) => (
                <Link key={story.slug} href={`/stories/${story.slug}`} className="group block rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-shadow bg-brand-white flex flex-col h-full">
                  <div className={`w-full h-52 ${story.color || "bg-brand-rust"} relative flex items-center justify-center shrink-0`}>
                    {story.mainImage ? (
