...
export default async function ExperienceDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Pick a few relevant destinations to show
  const relatedDests = destinations.slice(0, 4);
  ...
  return (
    <main className="min-h-screen bg-brand-white">
      ...
      {/* Hero */}
      <section className={`w-full ${exp.color} pt-36 pb-24 px-6 relative`}>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
            {relatedDests.map((dest) => (
              <Link key={dest.slug} href={`/destinations/${dest.slug}`} className={`${dest.color} rounded-2xl h-56 flex items-end p-6 group hover:opacity-90 transition-opacity`}>
                <div>
      ...
      <section className="bg-brand-white py-20 px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex gap-4 flex-wrap">
            {experiences.filter((e) => e.slug !== exp.slug).map((e) => (
              <Link key={e.slug} href={`/experiences/${e.slug}`} className={`${e.color} flex items-center gap-3 px-6 py-3 rounded-full text-brand-white font-heading font-bold uppercase tracking-wide text-sm hover:opacity-90 transition-opacity`}>
