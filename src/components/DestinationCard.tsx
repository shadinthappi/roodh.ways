...
export default function DestinationCard({ slug, name, tagline, region, categories, color, mainImage }: DestinationCardProps) {
  return (
    <motion.div whileHover={{ y: -6 }} transition={{ duration: 0.25, ease: "easeOut" }}>
      <Link href={`/destinations/${slug}`} className="group block rounded-2xl overflow-hidden shadow-md hover:shadow-2xl transition-shadow bg-brand-offwhite">
        {/* Image Placeholder */}
        <div className={`w-full h-56 ${color} relative flex items-center justify-center overflow-hidden`}>
          {mainImage ? (
            <Image src={urlFor(mainImage).url()} alt={name} fill unoptimized className="object-cover transition-transform duration-700 group-hover:scale-105" />
          ) : (
        ...
        <div className="p-6 group-hover:bg-brand-dark transition-colors duration-300">
          <div className="flex gap-2 flex-wrap mb-3">
            {categories.map((cat) => (
              <span key={cat} className="text-brand-blue group-hover:text-brand-sand text-xs font-bold uppercase tracking-widest font-sans transition-colors">{cat}</span>
            ))}
          </div>
          ...
          <span className="font-sans text-xs font-bold uppercase tracking-widest text-brand-blue group-hover:text-brand-sand transition-colors">
            Discover →
          </span>
        </div>
      </Link>
